...
void * udpWrapper::get_in_port(struct sockaddr *sa)
{
	if (sa->sa_family == AF_INET) {
...
int udpWrapper::beginRec(udpInfo info)
{
	...
	while (1)
	{
		...
		if (DEBUG) printf("port %s\n", inet_ntop(their_addr.ss_family, get_in_port((struct sockaddr *)&their_addr), p, sizeof p));
		string raw;
		raw.append(buf, numbytes);
		
		info.msg.clear();
		info.msg = udpDecode(raw);
		info.ip = inet_ntop(their_addr.ss_family, get_in_addr((struct sockaddr *)&their_addr), s, sizeof s);
		info.port = inet_ntop(their_addr.ss_family, get_in_port((struct sockaddr *)&their_addr), p, sizeof p);
